...
>We found this old robo caller. It basically generates random phone numbers to spam. We found the last list of numbers it generated and also some weird file... Maybe it's got to do with these new beta features they were testing?
...
* [`RoboCaller1337.py`](RoboCaller1337.py) - From the challenge description, we know this is code for a robo caller.
* [`robot_numbers_list.txt`](robot_numbers_list.txt) - This is a long list of previously-generated phone numbers. This must be useful somehow, otherwise why provide it.
* [`secret.enc`](secret.enc) - Presumably the flag, but encrypted somehow (probably by the robo-caller code).
...
* They are generating numbers using the `random` module without any modification besides the bit shift, which can be undone.
...
At the very least, importing `random` sets a new seed every time.
...
A-ha! The challenge does exactly this - use the PRNG for security purposes. Let's look further into this.
...
Let's write some code (see [solve.py](solve.py)):
