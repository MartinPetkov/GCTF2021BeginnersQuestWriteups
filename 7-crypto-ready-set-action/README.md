...
There's seemingly very little going on here. We get 2 primes (presumably in a cryptographically-secure way, as it's using a function from the [Crypto.Util](https://pycryptodome.readthedocs.io/en/latest/src/util/util.html) package), multiply them together, and raise the flag to the power of 3 modulo the product. We're also given the values of the power and the product.
